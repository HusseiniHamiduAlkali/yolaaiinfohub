    ...
    if (!naviSection.querySelector('.section-content')) {
        naviSection.innerHTML = `
            <div class="section-content">
                ...
                <div id="map-section">
                    <div id="stats-bar" style="display:none; background:#333; color:white; padding:10px; border-radius:8px; margin-bottom:10px; font-size:14px; display:flex; gap:15px; justify-content: center; width: 1000px; max-width: 100%; margin: 0 auto 10px auto;">
                        <div style="display:flex; flex-direction:column; align-items:center;">
                            <span style="font-size:10px; text-transform:uppercase; opacity:0.8;">Distance</span>
                            <span id="dist-val" style="font-weight:bold;">0 km</span>
                        </div>
                        <div style="display:flex; flex-direction:column; align-items:center;">
                            <span style="font-size:10px; text-transform:uppercase; opacity:0.8;">Est. Travel Time</span>
                            <span id="time-val" style="font-weight:bold;">0h 0m</span>
                        </div>
                    </div>
                    <div id="map" class="navi-map-styled"></div>
                    <div class="map-controls" style="margin-top: 10px; display: flex; gap: 10px; justify-content: center;">
                        <button onclick="window.togglePinMode()" id="pin-btn" style="padding: 10px 20px; border-radius: 8px; cursor: pointer; border: none; font-weight: bold; background: #3182ce; color: white;">📍 Pin on Map</button>
                        <button onclick="window.map.setView([9.2182, 12.4818], 13)" style="padding: 10px 20px; border-radius: 8px; cursor: pointer; border: none; font-weight: bold; background: #4a5568; color: white;">Center on Yola</button>
                    </div>
                </div>
            </div>
        `;
...
// --- CORE MAP ENGINE (TomTom Satellite) ---
window.initMap = function() {
    const mapDiv = document.getElementById('map') || document.getElementById('navi-map');
    if (!mapDiv) return;

    // Initialize Map centered on Yola
    window.map = L.map(mapDiv).setView([9.2182, 12.4818], 13);

    // TomTom Satellite Tiles
    L.tileLayer(`https://api.tomtom.com/map/1/tile/sat/main/{z}/{x}/{y}.jpg?key=${TOMTOM_KEY}`, {
        attribution: '© TomTom',
        maxZoom: 22
    }).addTo(window.map);
    ...
    window.pinMode = false;

    // Click Event for Pin Mode
    window.map.on('click', function(e) {
        if (window.pinMode) handleMapClick(e.latlng);
    });
};

// --- PIN ON MAP LOGIC ---
let pinStep = 0;
let manualStart, manualEnd;

window.togglePinMode = function() {
    window.pinMode = true;
    pinStep = 1;
    const btn = document.getElementById('pin-btn');
    btn.innerText = "Click Start Point...";
    btn.style.background = "#ed8936"; // Orange for active
};
...
async function handleMapClick(latlng) {
    if (pinStep === 1) {
        ...
        pinStep = 2;
        document.getElementById('pin-btn').innerText = "Click End Point...";
    } else if (pinStep === 2) {
        manualEnd = latlng;
        setMarker('end', latlng);
        window.pinMode = false;
        document.getElementById('pin-btn').innerText = "📍 Pin on Map";
        document.getElementById('pin-btn').style.background = "#3182ce";
        window.drawRoute(manualStart, manualEnd, true);
    }
}
...
function setMarker(type, latlng) {
    if (type === 'start') {
        ...
        window.startMarker = L.marker(latlng).addTo(window.map).bindPopup("Starting Point").openPopup();
    } else {
        if (window.endMarker) window.map.removeLayer(window.endMarker);
        window.endMarker = L.marker(latlng).addTo(window.map).bindPopup("Destination").openPopup();
    }
}

// --- NAVIGATION LOGIC ---
window.drawRoute = async function(origin, destination, isCoords = false) {
    try {
        let start = origin, end = destination;
        ...
        const routeUrl = `https://api.tomtom.com/routing/1/calculateRoute/${start.lat},${start.lng || start.lon}:${end.lat},${end.lng || end.lon}/json?key=${TOMTOM_KEY}&routeType=fastest&traffic=true`;
        ...
        document.getElementById('dist-val').innerText = (summary.lengthInMeters / 1000).toFixed(2) + " km";
        document.getElementById('time-val').innerText = formatTime(summary.travelTimeInSeconds);

        // Draw Route Line
        if (window.routeLine) window.map.removeLayer(window.routeLine);
        window.routeLine = L.polyline(points, {color: '#39ff14', weight: 6, opacity: 0.9}).addTo(window.map);
...
async function geocode(query) {
    ...
    const url = `https://api.tomtom.com/search/2/search/${encodeURIComponent(q)}.json?key=${TOMTOM_KEY}&countrySet=NG&limit=1`;
...
// --- ORIGINAL AI & TTS LOGIC PRESERVED ---
window.useGemini25 = localStorage.getItem('gemini_model_preference') === '2.5';
...
// ... Rest of the static HTML content (Transport Agents, Markets, etc.)
window.renderSection = function() {
    // [Inject the full HTML from your original file here]
    // Note: Use the section content from the original naviinfo.js snippet
    // including "Transport Agents", "Parks and Recreation", etc.
};
