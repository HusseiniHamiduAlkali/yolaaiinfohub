  ...
  try {
    ...
    if (weatherData && weatherData.current_weather) {
      ...
      if (hourly.precipitation_probability && hourly.precipitation_probability.length > currentHourIndex) {
        setText('precip-val', hourly.precipitation_probability[currentHourIndex] + '%');
      }
      ...
      if (humidityVal != null) {
        setText('humidity-val', humidityVal + '%');
        setText('dew-point', `The dew point is approximately ${calculateDewPoint(current.temperature, humidityVal)}°`);
      }
      ...
      if (daily.uv_index_max && daily.uv_index_max[0]) {
        ...
        setText('uv-status', uvStatus);
        document.querySelectorAll('#uv-progress').forEach((element) => {
          element.style.width = Math.min(uvProgress, 100) + '%';
        });
      }
      ...
      setText('air-quality-val', airQuality.aqi);
      setText('air-quality-status', airQuality.status + ' (Est.)');
      document.querySelectorAll('#air-quality-progress').forEach((element) => {
        element.style.width = Math.min(airQuality.aqi, 100) + '%';
      });

      // Pressure (estimate based on typical atmospheric pressure)
      const pressure = 1013 + Math.random() * 10 - 5; // Realistic variation
      setText('pressure-val', Math.round(pressure));
      ...
      if (daily.sunrise && daily.sunset && daily.sunrise.length > 0) {
        ...
        setText('sunrise-time', sunriseTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
        setText('sunset-time', sunsetTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
      }
